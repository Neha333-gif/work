...
def train_marketing_campaign_response_model():
    print("="*60)
    print("STARTING MARKETING CAMPAIGN RESPONSE MODEL TRAINING")
    print("="*60)

    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    DATA_PATH = os.path.join(BASE_DIR, '..', 'marketing_campaign_response_dataset', 'campaign_responses.csv')
    RESULTS_DIR = os.path.join(BASE_DIR, 'results')
    os.makedirs(RESULTS_DIR, exist_ok=True)

    if not os.path.exists(DATA_PATH):
        raise FileNotFoundError(f"Dataset not found at {DATA_PATH}. Please ensure campaign_responses.csv is in the marketing_campaign_response_dataset folder.")

    print("[INFO] Loading campaign response dataset...")
    ...
    if 'responded' not in df.columns:
        raise ValueError("Target column 'responded' not found in dataset.")

    X = df.drop(columns=['responded'])
    y = LabelEncoder().fit_transform(df['responded'])
    target_classes = ['No', 'Yes']

    cat_cols = [c for c in ['gender', 'employed', 'marital_status'] if c in X.columns]
    num_cols = [c for c in ['age', 'annual_income', 'credit_score', 'no_of_children'] if c in X.columns]
    ...
    metadata_path = os.path.join(BASE_DIR, 'marketing_campaign_response_prediction_metadata.joblib')
    ...
    sns.countplot(x=df['responded'], palette=['#ff7f0e', '#1f77b4'])
    plt.title('Campaign Response Class Distribution')
    ...
    plt.figure(figsize=(8, 6))
    numeric_df = df[num_cols + ['responded']].copy()
    numeric_df['responded'] = LabelEncoder().fit_transform(numeric_df['responded'])
    ...
    report_text = f"""Marketing Campaign Response Prediction Model Evaluation Report
=================================================

Pipeline: Random Forest Classification for campaign response prediction

Dataset shape: {df.shape}
Target distribution:
{df['responded'].value_counts().to_string()}
...
"""
    with open(os.path.join(RESULTS_DIR, 'marketing_campaign_response_prediction_evaluation.txt'), 'w', encoding='utf-8') as f:
        f.write(report_text)

    sample_df = df.head(100).copy()
    sample_df['predicted_response'] = ["Yes" if p == 1 else "No" for p in rf_pipeline.predict(sample_df.drop(columns=['responded']))]
    sample_df['conversion_probability'] = rf_pipeline.predict_proba(sample_df.drop(columns=['responded']))[:, 1]
    sample_df.to_csv(os.path.join(RESULTS_DIR, 'sample_marketing_campaign_response_predictions.csv'), index=False)

    print("[OK] Evaluation report saved successfully.")
    print("[OK] Sample predictions saved successfully.")
    print("\n" + "="*60)
    print("MARKETING CAMPAIGN RESPONSE MODEL TRAINING COMPLETE")
    print("="*60)


if __name__ == '__main__':
    train_marketing_campaign_response_model()
