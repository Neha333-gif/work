...
app = FastAPI(title="Insurance Claim Prediction API", version="1.0.0")
...
def load_models():
    """Load the insurance claim prediction pipeline and metadata from disk."""
    global pipeline, metadata
    try:
        BASE_DIR = os.path.dirname(os.path.abspath(__file__))
        pipeline_path = os.path.join(BASE_DIR, "insurance_claim_prediction_pipeline.joblib")
        meta_path = os.path.join(BASE_DIR, "insurance_claim_prediction_metadata.joblib")

        if os.path.exists(pipeline_path):
            pipeline = joblib.load(pipeline_path)
            if os.path.exists(meta_path):
                metadata = joblib.load(meta_path)
            print("[INFO] Insurance Claim Prediction pipeline loaded successfully.")
        else:
            print("[WARNING] Model pipeline not found. Please run insurance_claim_prediction_trainer.py first to train the model.")
...
class PredictRequest(BaseModel):
    features: dict = Field(..., description="Feature dictionary for a single insurance claim input row")


@app.get("/", response_class=HTMLResponse)
def serve_frontend():
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    html_path = os.path.join(BASE_DIR, "insurance_claim_prediction_frontend.html")
    if not os.path.exists(html_path):
        return HTMLResponse(content="<h1>Frontend UI not found. Please ensure insurance_claim_prediction_frontend.html exists in this directory.</h1>")
...
def health():
    ...
    return {
        "status": status,
        "model": f"Insurance Claim Predictor ({model_type})",
        "assets_exist": {
            "pipeline": os.path.exists(os.path.join(BASE_DIR, "insurance_claim_prediction_pipeline.joblib")),
            "metadata": os.path.exists(os.path.join(BASE_DIR, "insurance_claim_prediction_metadata.joblib")),
        }
    }


@app.post("/predict")
def predict_claim(req: PredictRequest):
    if pipeline is None:
...
async def upload_csv_and_predict(file: UploadFile = File(...)):
    ...
    try:
        ...
        df_input["predicted_claim_status"] = [metadata.get("target_classes", ["No Claim", "Claim"])[int(p)] for p in preds]
        if hasattr(pipeline, "predict_proba"):
            proba = pipeline.predict_proba(df_input.drop(columns=["predicted_claim_status"], errors="ignore"))
            df_input["claim_approval_probability"] = [float(x[1]) for x in proba]

        stream = io.StringIO()
        df_input.to_csv(stream, index=False)
        response = StreamingResponse(iter([stream.getvalue()]), media_type='text/csv')
        response.headers['Content-Disposition'] = 'attachment; filename=insurance_claim_prediction_results.csv'
        return response
    except Exception as e:
...
def get_metrics():
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    metrics_file = os.path.join(BASE_DIR, "results", "insurance_claim_prediction_evaluation.txt")
    ...
    return {
        ...
        "plots": {
            "class_distribution": "/results/image/claim_class_distribution.png",
            ...
            "prediction_distribution": "/results/image/claim_probability_distribution.png",
        }
    }
...
def background_retrain():
    print("[RETRAIN] Starting background insurance claim model retraining...")
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    script_path = os.path.join(BASE_DIR, "insurance_claim_prediction_trainer.py")
    try:
...
if __name__ == "__main__":
    import uvicorn
    print("\n" + "="*60)
    print("STARTING INSURANCE CLAIM PREDICTION API SERVER")
    print("="*60)
